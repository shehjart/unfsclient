 ...
 * 02111-1307, USA.
 *
 * $Id: hash_table.c 8467 2006-06-04 12:21:25Z ska $
 *
 ********************************************************************/
...
static void remove_all_entries(ght_hash_table_t *p_ht)
{
  int i;
  assert(p_ht);
...
/* Empty out a hash table */
void ght_remove_all(ght_hash_table_t *p_ht)
{
  assert(p_ht);

  /* Remove the entries but do not free the memory alloced
   * for the bucket array.
   */
  remove_all_entries(p_ht);

  /* Re-init the count of entries for each bucket. */
  if (p_ht->p_nr)
    memset(p_ht->p_nr, 0, p_ht->i_size*sizeof(int));
