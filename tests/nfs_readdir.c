...
void nfs_readdir_cb(void *msg, int len, void *priv_ctx)
{
	READDIR3res *res = NULL;
	entry3 *dirent = NULL;

	res = xdr_to_READDIR3res(msg, len);
	if(res == NULL)
		return;

	fprintf(stdout, "Inside readdir callback..\n");
	if(res->status != NFS3_OK) {
		fprintf(stdout, "Readdir callback failed..\n");
		return;
	}
	
	fprintf(stdout, "NFS READDIR Reply received: Msg Size: %d\n", len);
	dirent = res->READDIR3res_u.resok.reply.entries;
...
int main(int argc, char *argv[])
{
	...
	if(argc < 4) {
		fprintf(stderr, "Not enough arguments\nThis tests the asynchronous "
				"libnfsclient NFS READDIR call interface and "
				"callback\n"
				"USAGE: nfs_readdir <server> <remote_file_dir> "
				"<dirname>\n");
		return 0;
	}
	...
	fprintf(stdout, "Received lookup reply..\n");
	rdir.dir.data.data_len = lfh.fhandle3_len;
	rdir.dir.data.data_val = lfh.fhandle3_val;
	rdir.cookie = 0;
	memset(&rdir.cookieverf, 0, NFS3_COOKIEVERFSIZE);
	rdir.count = 1500;
	fprintf(stdout, "Sending readdir call..\n");
	stat = nfs3_readdir(&rdir, ctx, nfs_readdir_cb, NULL);
	if(stat == RPC_SUCCESS)
		fprintf(stderr, "NFS READDIR Call sent successfully\n");
	else {
		fprintf(stderr, "Could not send NFS READDIR call\n");
		return 0;
	}

	fprintf(stdout, "Waiting for readdir reply..\n");
	nfs_complete(ctx, RPC_BLOCKING_WAIT);
	fprintf(stdout, "Received readdir reply..\n");
	return 0;
}
