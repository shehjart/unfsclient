...
void nfs_getattr_cb(void *msg, int len, void *priv_ctx)
{
	GETATTR3res *res = NULL;

	res = xdr_to_GETATTR3res(msg, len);
	if(res == NULL)
		return;

	fprintf(stdout, "Inside getattr callback..\n");
	if(res->status != NFS3_OK) {
		fprintf(stdout, "Getattr call failed..\n");
		free_GETATTR3res(res);
		return;
	}

	fprintf(stderr, "NFS GETATTR Reply received: Size: %d\n", len);
	free_GETATTR3res(res);
	return;
}
...
void nfs_lookup_cb(void *msg, int len, void *priv_ctx)
{
	...
	/* Prepare tp copy looked up file handle. */
	...
	fprintf(stdout, "NFS LOOKUP reply received: FH size: %d\n", fh_len);
}
...
int main(int argc, char *argv[])
{
	...
	LOOKUP3args largs;
	GETATTR3args gattr_args;

	if(argc < 4) {
		fprintf(stderr, "Not enough arguments\nThis tests the asynchronous "
				"libnfsclient NFS GETATTR call interface and "
				"callback\n"
				"USAGE: nfs_getattr <server> <remote_file_dir> "
				"<file/dirname>\n");
		return 0;
	}
	...
	gattr_args.object.data.data_len = lfh.fhandle3_len;
	gattr_args.object.data.data_val = lfh.fhandle3_val;
	fprintf(stdout, "Sending getattr call..\n");
	stat = nfs3_getattr(&gattr_args, ctx, nfs_getattr_cb, NULL);
	if(stat == RPC_SUCCESS)
		fprintf(stderr, "NFS GETATTR Call sent successfully\n");
	else {
		fprintf(stderr, "Could not send NFS GETATTR call\n");
		return 0;
	}

	fprintf(stdout, "Waiting for getattr reply..\n");
	nfs_complete(ctx, RPC_BLOCKING_WAIT);
	fprintf(stdout, "Received getattr reply..\n");
	return 0;
}
