...
void nfs_link_cb(void *msg, int len, void *priv_ctx)
{
	LINK3res *res = NULL;

	res = xdr_to_LINK3res(msg, len);
	if(res == NULL)
		return;
	
	fprintf(stdout, "Inside link callback..\n");
	if(res->status != NFS3_OK) {
		fprintf(stdout, "Link call failed..\n");
		free_LINK3res(res);
		return;
	}

	fprintf(stderr, "NFS LINK Reply received: Msg Size: %d\n", len);
	free_LINK3res(res);
