...
void nfs_pathconf_cb(void *msg, int len, void *priv_ctx)
{
	PATHCONF3res *res = NULL;

	res = xdr_to_PATHCONF3res(msg, len);
	if(res == NULL)
		return;

	fprintf(stdout, "Inside pathconf callback..\n");
	if(res->status != NFS3_OK) {
		fprintf(stdout, "Pathconf call failed..\n");
		free_PATHCONF3res(res);
		return;
	}

	fprintf(stderr, "NFS PATHCONF Reply received: Msg Size: %d\n", len);
	free_PATHCONF3res(res);
	return;
}
...
int main(int argc, char *argv[])
{
	...
	if(argc < 3) {
		fprintf(stderr, "Not enough arguments\nThis tests the asynchronous "
				"libnfsclient NFS PATHCONF call interface and "
				"callback\n"
				"USAGE: nfs_pathconf <server> <remote_file_dir>\n");
		return 0;
	}
	...
	pc.object.data.data_len = mntfh.fhandle3_len;
	pc.object.data.data_val = mntfh.fhandle3_val;
	fprintf(stdout, "Sending pathconf call..\n");
	stat = nfs3_pathconf(&pc, ctx, nfs_pathconf_cb, NULL);
	if(stat == RPC_SUCCESS)
		fprintf(stderr, "NFS PATHCONF Call sent successfully\n");
	else {
		fprintf(stderr, "Could not send NFS PATHCONF call\n");
		return 0;
	}

	fprintf(stdout, "Waiting for pathconf reply..\n");
	nfs_complete(ctx, RPC_BLOCKING_WAIT);

	fprintf(stdout, "Received pathconf reply..\n");
	return 0;
}
