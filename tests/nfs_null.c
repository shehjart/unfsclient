...
void nfs_null_cb(void *m, int len, void *p)
{
	fprintf(stderr, "NFS NULL reply received: Size: %d\n",
			len);
}
...
int main(int argc, char *argv[])
{
	...
	if(argc < 2) {
		fprintf(stderr, "Not enough arguments\nThis tests the asynchronous libnfsclient NFS NULL call interface and callback\n"
				"USAGE: nfs_null <server>\n");
		return 0;
	}
	...
	ctx = nfs_init((struct sockaddr_in *)srv_addr->ai_addr, IPPROTO_TCP,
	...
	stat = nfs3_null(ctx, nfs_null_cb, NULL);
	if(stat == RPC_SUCCESS)
		fprintf(stderr, "NFS NULL Call sent successfully\n");
	else
		fprintf(stderr, "Could not send NFS NULL call\n");
