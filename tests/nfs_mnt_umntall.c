...
void mnt_umntall_cb(void *m, int len, void *p)
{
	fprintf(stderr, "Mount UMNTALL reply received: Size: %d\n",
			len);
}
...
int main(int argc, char *argv[])
{
	...
	if(argc < 2) {
		fprintf(stderr, "Not enough arguments\nThis tests the asynchronous libnfsclient MOUNT UMNTALL call interface and callback\n"
				"USAGE: nfs_mnt_umntall <server>\n");
		return 0;
	}
	...
	stat = mount3_umntall(ctx, mnt_umntall_cb, NULL);
	if(stat == RPC_SUCCESS)
		fprintf(stderr, "MOUNT UMNTALL Call sent successfully\n");
	else
		fprintf(stderr, "Could not send MOUNT UMNTALL call\n");
