...
fhandle3 mntfh;
void nfs_symlink_cb(void *msg, int len, void *priv_ctx)
{
	SYMLINK3res *res = NULL;
	int fh_len;

	res = xdr_to_SYMLINK3res(msg, len);
	if(res == NULL)
		return;
	
	fprintf(stdout, "Inside symlink callback..\n");
	if(res->status != NFS3_OK) {
		fprintf(stdout, "Symlink call failed..\n");
		free_SYMLINK3res(res);
		return;
	}

	fh_len = res->SYMLINK3res_u.resok.obj.post_op_fh3_u.handle.data.data_len;
	fprintf(stderr, "NFS SYMLINK Reply received: FH Size: %d\n", fh_len);
	free_SYMLINK3res(res);
...
void nfs_mnt_cb(void *msg, int len, void *priv_ctx)
{
	...
	fprintf(stdout, "Inside mount call..\n");
