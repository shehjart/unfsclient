...
void nfs_setattr_cb(void *msg, int len, void *priv_ctx)
{
	SETATTR3res *res = NULL;

	res = xdr_to_SETATTR3res(msg, len);
	if(res == NULL)
		return;

	fprintf(stdout, "Inside setattr callback..\n");
	if(res->status != NFS3_OK) {
		fprintf(stdout, "Setattr call failed..\n");
		free_SETATTR3res(res);
		return;
	}

	fprintf(stderr, "NFS SETATTR Reply received: Size: %d\n", len);
	free_SETATTR3res(res);
	return;
}
...
int main(int argc, char *argv[])
{
	...
	if(argc < 4) {
		fprintf(stderr, "Not enough arguments\nThis tests the asynchronous "
				"libnfsclient NFS SETATTR call interface and "
				"callback\n"
				"USAGE: nfs_setattr <server> <remote_file_dir> "
				"<file/dirname>\n");
		return 0;
	}
	...
	sa.object.data.data_len = lfh.fhandle3_len;
	sa.object.data.data_val = lfh.fhandle3_val;
	memset(&sa.new_attributes, 0, sizeof(sattr3));
	memset(&sa.guard, 0, sizeof(sattrguard3));
	sa.new_attributes.mode.set_it = 1;
	sa.new_attributes.uid.set_it = 1;
	sa.new_attributes.gid.set_it = 1;
	sa.new_attributes.size.set_it = 1;
	sa.new_attributes.atime.set_it = 1;
	sa.new_attributes.mtime.set_it = 1;

	fprintf(stdout, "Sending setattr call..\n");
	stat = nfs3_setattr(&sa, ctx, nfs_setattr_cb, NULL);
	if(stat == RPC_SUCCESS)
		fprintf(stderr, "NFS SETATTR Call sent successfully\n");
	else {
		fprintf(stderr, "Could not send NFS SETATTR call\n");
		return 0;
	}

	fprintf(stdout, "Waiting for setattr reply..\n");
	nfs_complete(ctx, RPC_BLOCKING_WAIT);
