...
void nfs_mkdir_cb(void *msg, int len, void *priv_ctx)
{
	MKDIR3res *res = NULL;
	int fh_len;

	res = xdr_to_MKDIR3res(msg, len);
	if(res == NULL)
		return;
	
	fprintf(stdout, "Inside mkdir callback..\n");
	if(res->status != NFS3_OK) {
		fprintf(stdout, "Mkdir call failed..\n");
		free_MKDIR3res(res);
		return;
	}

	fh_len = res->MKDIR3res_u.resok.obj.post_op_fh3_u.handle.data.data_len;
	fprintf(stderr, "NFS MKDIR Reply received: FH Size: %d\n", fh_len);
	free_MKDIR3res(res);
...
int main(int argc, char *argv[])
{
	...
	if(argc < 4) {
		fprintf(stderr, "Not enough arguments\nThis tests the asynchronous "
				"libnfsclient NFS MKDIR call interface and "
				"callback\n"
				"USAGE: nfs_mkdir <server> <remote_file_dir> "
				"<new dirname>\n");
		return 0;
	}
	...
	mk.where.dir.data.data_len = mntfh.fhandle3_len;
	mk.where.dir.data.data_val = mntfh.fhandle3_val;
	mk.where.name = argv[3];
	memset(&mk.attributes, 0, sizeof(sattr3));
	mk.attributes.mode.set_it = 1;
	mk.attributes.uid.set_it = 1;
	mk.attributes.gid.set_it = 1;
	mk.attributes.size.set_it = 1;
	mk.attributes.atime.set_it = 1;
	mk.attributes.mtime.set_it = 1;

	fprintf(stdout, "Sending mkdir call..\n");
	stat = nfs3_mkdir(&mk, ctx, nfs_mkdir_cb, NULL);
	if(stat == RPC_SUCCESS)
		fprintf(stderr, "NFS MKDIR Call sent successfully\n");
	else {
		fprintf(stderr, "Could not send NFS MKDIR call\n");
		return 0;
	}

	fprintf(stdout, "Waiting for mkdir reply..\n");
	nfs_complete(ctx, RPC_BLOCKING_WAIT);
	fprintf(stdout, "Received mkdir reply..\n");
	return 0;
}
