 ...
 * 02111-1307, USA.
 *
 * $Id: ght_hash_table.h.in 8462 2006-06-04 08:14:23Z ska $
 *
 ********************************************************************/
...
/**
 *
 * Remove all the entries in the hashtable and free the mmeory used by
 * the entries. Note that only the memory used by the entries is
 * freed. This is different from ght_finalize because, here we do not
 * free the memory allocated to the table, in effect, allowing the
 * table to be re-used without a following ght_create. Table size
 * remains the same.
 *
 *
 * @param p_ht the table whose entries are to be removed.
 */
void ght_remove_all(ght_hash_table_t *p_ht);
